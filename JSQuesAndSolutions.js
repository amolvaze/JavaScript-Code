...
export function insertAtPosition(element, list, position) {
  ...
  for (let i = 0; i < list.length; i++) {
    if (i === position) result.push(element);
    result.push(list[i]);
  }
  return result;
}
...
export function getIndexedObject(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    if (items[i].props) {
      result[items[i].name] = items[i].props;
    }
  }
  return result;
}
...
export function sumOfValuesMoreThanN(items, n) {
  ...
  let result = [];
  for (let i = 0; i < items.length; i++) {
    if (items[i].values.reduce((a, b) => a + b, 0) > n) {
...
export function promiseByComparison(a, b, ms) {
  ...
  let myPromise = new Promise(function (myResolve, myReject) {
    ...
    else if (a < b) result = 'b is greater than a';
    else result = 'a and b are equal';
    setTimeout(function () {
      myResolve(result);
    }, ms);
  });
